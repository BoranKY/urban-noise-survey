...
if selected is None:
    ...
    if submit:
        ...
        if errors:
            ...
            # Build response dict (for now: just show it; later you can send to Google Forms)
            response = {
                # Meta
                ...
                # Street prediction info
                "osmid": str(selected.get("osmid", "")),
                "highway": highway,
                "pred_label": pred_label,
                "pred_score": pred_score,
                # User rating for this street
                ...
                # Optional user geolocation (browser-based)
                "user_lat": user_lat,
                "user_lon": user_lon,
            }

            st.sidebar.success("✅ Thank you! Your response has been recorded in this session.")

            # Debug / development: show the collected response in the main page
            st.subheader("Current response (for debugging / export later)")
            st.json(response)
